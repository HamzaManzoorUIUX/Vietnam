...
export default ({ img, title, location, fetures, price, type }) => {
    const PriceCom = () => {
        if (type == 1) {
            return price;
        }
    ...
    return (<CardActionArea><div>
        ...
        <div className="txt-algn-lft bg-white pl-4 pt-4">
            ...
            <div className="dsp-flex"> {fetures.map(x => <FeatureFun name={x.name} val={x.val} />)}</div>
        </div>
    </CardActionArea>)
}